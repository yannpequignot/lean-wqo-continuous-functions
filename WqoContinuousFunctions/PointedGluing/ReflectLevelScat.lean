...
The next three declarations assemble `bad_pairseq_restricts_to_level` and
`no_bad_pairseq_level` into the full theorem.

**Proof architecture.**

1. `no_bad_pairseq_all_levels` — transfinite induction on `β < ω₁`:
   assume no bad sequence exists at any level `γ < β` (induction hypothesis),
   show no bad sequence exists at level `β`.
   The inductive step consists of providing the `ih` that `no_bad_pairseq_level` expects:
   if a bad `PairSeq (ScatFun_level_lt β)` existed, we would coerce it to
   `PairSeq ScatFun`, apply `bad_pairseq_restricts_to_level` to land at some level `γ`,
   observe `γ < β` from the membership bound, and contradict the induction hypothesis.

2. `TwoBQO_scatReduces` — if a bad `PairSeq ScatFun` existed,
   `bad_pairseq_restricts_to_level` would produce a bad sequence at some level `β < ω₁`,
   contradicting `no_bad_pairseq_all_levels β`.

3. `MainTheorem3_ScatFun` — WQO corollary via `TwoBQO.wellQuasiOrdered`.
-/

/-- **Auxiliary (transfinite induction).** For every `β < ω₁`, there is no bad
pair-sequence in `ScatFun_level β`. The proof is by strong transfinite induction
on `β`, using `no_bad_pairseq_level` as the inductive step. -/
theorem no_bad_pairseq_all_levels (β : Ordinal.{0}) (hβ : β < omega1) :
    ¬ ∃ f : PairSeq (ScatFun_level β), BadPairSeq (scatReduces_level β) f := by
  revert hβ
  induction β using Ordinal.induction with
  | h β ih_β =>
    intro hβ
    -- Apply no_bad_pairseq_level; it suffices to supply the inductive hypothesis,
    -- i.e., that no bad pair-sequence exists in ScatFun_level_lt β.
    apply no_bad_pairseq_level β hβ
    intro ⟨f, hbad⟩
    -- Coerce f : PairSeq (ScatFun_level_lt β) to a PairSeq ScatFun by forgetting the bound.
    let g : PairSeq ScatFun := fun m n h => (f m n h).val
    have hbad_g : BadPairSeq scatReduces g :=
      fun m n l hmn hnl hrel => hbad m n l hmn hnl hrel
    -- bad_pairseq_restricts_to_level gives a level γ < ω₁ and a bad restriction there.
    obtain ⟨γ, hγ_ω1, e, he, hbad_γ, hrank⟩ := bad_pairseq_restricts_to_level g hbad_g
    ...
    let f_γ : PairSeq (ScatFun_level γ) :=
      fun m n h => ⟨restrictPairSeq g e he m n h, hrank m n h⟩
...
**Proof.** If a bad `PairSeq ScatFun` existed, `bad_pairseq_restricts_to_level`
would produce a bad sequence at some level `β < ω₁`, contradicting
`no_bad_pairseq_all_levels`. -/
theorem TwoBQO_scatReduces : TwoBQO scatReduces := by
  rw [isTwoBQO_iff]
  intro ⟨f, hbad⟩
  obtain ⟨β, hβ, e, he, hbad_level, hrank⟩ := bad_pairseq_restricts_to_level f hbad
  let f_β : PairSeq (ScatFun_level β) :=
    fun m n h => ⟨restrictPairSeq f e he m n h, hrank m n h⟩
  exact no_bad_pairseq_all_levels β hβ
...
`A : Set (ℕ → ℕ)`. It follows immediately from the stronger `TwoBQO_scatReduces`. -/
theorem MainTheorem3_ScatFun : WellQuasiOrdered scatReduces :=
  TwoBQO.wellQuasiOrdered TwoBQO_scatReduces
