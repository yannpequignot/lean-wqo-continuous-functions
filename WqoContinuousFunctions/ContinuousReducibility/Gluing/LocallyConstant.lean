...
This file contains results about locally constant functions (Proposition 2.24) and
finite generation (Proposition 2.23).

(Fact 2.25 on infinite discrete subspaces now lives in
`GeneralTopology.DiscreteSubspaces`.)
...
theorem id_nat_reduces_locally_constant {X Y : Type*}
  ...
  obtain ⟨τ, hτ⟩ : ∃ τ : Y → ℕ, ContinuousOn τ (Set.range (f ∘ σ)) ∧ ∀ n, τ (f (σ n)) = n := by
    ...
    have hτ_cont : ∀ y ∈ Set.range (f ∘ σ), ∃! n : ℕ, f (σ n) = y := by
      simp +decide only [mem_range, comp_apply, hσ, forall_exists_index, forall_apply_eq_imp_iff]
